...
class QRCodeScannerViewController : UIViewController {
    ...
    @IBAction func changeSourceButtonClicked(_ sender: Any) {
        if useSecondApi {
            self.useSecondApi = false
            self.sourceButton.setTitle("1st API", for: .normal)
        } else {
            self.useSecondApi = true
            self.sourceButton.setTitle("2nd API", for: .normal)
...
extension QRCodeScannerViewController : AVCaptureMetadataOutputObjectsDelegate {
    ...
    func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        ...
        if supportedCodeTypes.contains(metadataObj.type) {
            ...
            if metadataObj.stringValue != nil {
                ...
                if self.useSecondApi {
                    self.postRequest(data: metadataObj.stringValue!)
                } else {
                    self.getRequest(str: metadataObj.stringValue!)
                }
                self.state = .editing
    ...
    func getRequest(str: String) {
        let url = "https://qrtests.herokuapp.com/receipts/get?\(str)"
